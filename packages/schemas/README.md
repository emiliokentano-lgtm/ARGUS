...
Kapitel 3 des Konzepts fordert *ein* Datenmodell für alles: ein Flugzeug, ein
...
```sh
make setup     # Werkzeuge installieren (npm + uv-Venv)
...
* **ts-proto**, **protobufjs**, **typescript** — TypeScript-Erzeugung und
  Übersetzungsprüfung.
...
Ein Feld in `required.json` aufzunehmen ist eine brechende Änderung *für
Produzenten* und braucht einen CHANGELOG-Eintrag (siehe §5.4).
